...
interface DashboardPageUIProps {
  ...
  onSelectLocations: (ids: number[]) => void;
  onDeselectLocations: (ids: number[]) => void;
  onClearSelection: () => void;
  stats: {
    totalAssets: number;
    activeFixed: number;
    activeConsumable: number;
    ...
    <div className="w-full">
      ...
      >
        ...
        <div
          ...
          className="mr-2 text-primary hover:scale-110 transition-transform"
        >
          {isSelected ? (
            <CheckSquare size={16} />
          ) : (
            <Square size={16} className="text-muted-foreground opacity-50" />
          )}
        </div>
        ...
        <Badge
          variant="outline"
          className={`text-[9px] px-1.5 py-0 uppercase ${isSelected ? "border-primary/30" : ""}`}
        >
          {node.level}
        </Badge>
      </div>
              ...
              <div className="space-y-1 pb-10">
                {locationTree.map((node: any) => (
                  <LocationTreeNode
                    ...
                    onDeselect={onDeselectLocations}
                  />
                ))}
              </div>
            )}
            ...
            {visibleAssets.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-16 text-center h-full">
                ...
                <h3 className="text-xl font-bold tracking-tight">
                  Zero Arrays Detected
                </h3>
                <p className="text-muted-foreground max-w-sm mt-2 text-sm font-medium">
                  There are no operational or severed network components inside
                  this specific geometrical footprint.
                </p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-3">
                        <div
                          className={`p-2 rounded-lg border shadow-sm ${asset.type === "consumable" ? "bg-orange-500/10 border-orange-500/20" : "bg-blue-500/10 border-blue-500/20"}`}
                        >
                      ...
                      <h3 className="font-bold text-[15px] leading-tight group-hover:text-primary transition-colors pr-4">
                        {asset.name}
                      </h3>
                        ...
                        <div>
                          ...
                          <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.max(0, (asset.quantity / asset.initialQuantity) * 100)}%`,
                              }}
                            />
                          </div>
                        </div>
